...
SET 'table.connector.kafka.consumer.fetch.min.bytes' = '1';
SET 'table.connector.kafka.consumer.fetch.max.wait.ms' = '500';
SET 'table.connector.kafka.consumer.max.partition.fetch.bytes' = '1048576';
SET 'table.connector.kafka.consumer.receive.buffer.bytes' = '32768';
SET 'table.connector.kafka.consumer.send.buffer.bytes' = '131072';
...
CREATE TABLE JoinResultsMJ (
                             ...
                             payment_id BIGINT,
                             inventory_id BIGINT,
                             PRIMARY KEY (tenant_id, supplier_id, product_id, category_id, order_id, customer_id, warehouse_id, shipping_id, payment_id, inventory_id) NOT ENFORCED
) WITH (
      'connector' = 'blackhole'
      );
...
SELECT DISTINCT tenant_id, supplier_id, product_id, category_id, order_id, customer_id, warehouse_id, shipping_id, payment_id, inventory_id
FROM (
         SELECT
         ...
         FROM TenantKafka t
                  LEFT JOIN (
             SELECT DISTINCT tenant_id, supplier_id, large_array
             FROM SuppliersKafka
         ) s ON t.tenant_id = s.tenant_id
                  LEFT JOIN (
             SELECT DISTINCT tenant_id, product_id, large_array
             FROM ProductsKafka
         ) p ON t.tenant_id = p.tenant_id
                  LEFT JOIN (
             SELECT DISTINCT tenant_id, category_id, large_array
             FROM CategoriesKafka
         ) c ON t.tenant_id = c.tenant_id
                  LEFT JOIN (
             SELECT DISTINCT tenant_id, order_id, large_array
             FROM OrdersKafka
         ) o ON t.tenant_id = o.tenant_id
                  LEFT JOIN (
             SELECT DISTINCT tenant_id, customer_id, large_array
             FROM CustomersKafka
         ) cust ON t.tenant_id = cust.tenant_id
                  LEFT JOIN (
             SELECT DISTINCT tenant_id, warehouse_id, large_array
             FROM WarehousesKafka
         ) w ON t.tenant_id = w.tenant_id
                  LEFT JOIN (
             SELECT DISTINCT tenant_id, shipping_id, large_array
             FROM ShippingKafka
         ) sh ON t.tenant_id = sh.tenant_id
                  LEFT JOIN (
             SELECT DISTINCT tenant_id, payment_id, large_array
             FROM PaymentKafka
         ) pay ON t.tenant_id = pay.tenant_id
                  LEFT JOIN (
             SELECT DISTINCT tenant_id, inventory_id, large_array
             FROM InventoryKafka
         ) i ON t.tenant_id = i.tenant_id
         WHERE
